...
pub async fn welcome(ctx: &serenityy::Context, new_member: &serenityy::Member, data: &Data) {
    ...
    let title = string_to_text(
        &d.title.unwrap_or("Welcome to <name(guild)>".to_string()),
        new_member,
        guild.clone(),
    );
    let description = string_to_text(
        &d.description
            .unwrap_or("Welcome <mantion(user)>".to_string()),
        new_member,
        guild.clone(),
    );
    ...
    let footertext = string_to_text(
        &footer.text.unwrap_or("<join_at(user)>".to_string()),
        new_member,
        guild.clone(),
    );
...
fn string_to_text(s: &String, user: &serenityy::Member, guild: Guild) -> std::string::String {
    s.replace("<mantion(user)>", format!("<@{}>", user.user.id).as_str())
        ...
        .replace(
            "<join_at(user)>",
            user.joined_at.unwrap().to_string().as_str(),
        )
        .replace("<none>", "")
...
fn text_to_img(s: &String, user: &serenityy::Member, guild: Guild) -> std::string::String {
    ...
    .replace(
        "!img(user)",
        user.user.avatar_url().unwrap_or("non".to_string()).as_str(),
    )
    .replace("!img(none)", "")
}
