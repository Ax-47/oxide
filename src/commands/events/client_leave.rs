use poise::serenity_prelude::{self as serenityy, GuildId,Guild};
use poise::serenity_prelude::ChannelId;
use serenity::utils::Colour;
use crate::database::models::{Data, RGB, self,TextwithICon};
pub async fn see_you_next_time(ctx:&serenityy::Context,guild_id:&GuildId,user: &serenityy::User,data:&Data){
    ...
    let title=string_to_text(&d.title.unwrap_or("See u nekst time".to_string()),user,guild.clone());
    let description=string_to_text(&d.description.unwrap_or("See u nekst time <mantion(user)>".to_string()),user,guild.clone());
    ...
    let thumbnail=text_to_img(&d.thumbnail.unwrap_or("!img(guild)".to_string()),user,guild.clone());
    
    let auth=d.auth.unwrap_or(TextwithICon::default());
    let authtext=string_to_text(&auth.text.unwrap_or("<name(user)>".to_string()),user,guild.clone());
    let authicon=text_to_img(&auth.icon.unwrap_or("!img(user)".to_string()),user,guild.clone());
    let footer=d.footer.unwrap_or(TextwithICon::default());
    let footertext=string_to_text(&footer.text.unwrap_or("<none>".to_string()),user,guild.clone());
    let footericon=text_to_img(&footer.icon.unwrap_or("!img(none)".to_string()),user,guild.clone());
...
fn string_to_text(s:&String,user:&serenityy::User,guild:Guild)-> std::string::String{
    s
    .replace("<mantion(user)>", format!("<@{}>",user.id).as_str())
...
fn text_to_img(s:&String,user:&serenityy::User,guild:Guild)-> std::string::String{
    ...
    .replace("!img(user)", user.avatar_url().unwrap_or("non".to_string()).as_str())
    .replace("!img(none)", "")
}